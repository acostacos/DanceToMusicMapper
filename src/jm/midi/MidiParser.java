...
/**
 * A MIDI parser
 * @author Andrew Sorensen, Sean T. Hayes and Tim Opie
 */
...
//                        }
	...
	private static void noteOn(int phrIndex, Note[] curNote,SMF smf,int i,
		...
		if((startTime > currentLength[phrIndex])&&
			...
			if(newTime < 0.25){
				double length=
				//	curNote[phrIndex].getDuration();	JythonMusic extension
				//curNote[phrIndex].setDuration(		JythonMusic extension
				...
				// restNote.setLength(newTime);			JythonMusic extension
			...
			while(partEnum.hasMoreElements()) {
				...
				while(phraseEnum.hasMoreElements()){
					...
					if(pitch != REST) {
						...
						// double endTime = startTime + (note.getLength() * phraseTempoMultiplier); JythonMusic extension
					...
					// startTime += tickRounder(note.getDuration() * phraseTempoMultiplier); //JythonMusic extension

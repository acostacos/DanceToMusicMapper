...
* @author Andrew Sorensen, Andrew Brown and Tim Opie
...
public class CPhrase implements JMC, Cloneable, Serializable{
	...
	*/
	...
	/* JythonMusic extension
	 * @param duration double xxxxxxxx
	public void addChord(int[] pitchArray, double duration){
		this.addChord(pitchArray,duration,MF);
	}
	*/
	...
	* ///// @param duration short		// JythonMusic extension commented out
	* @param dynmaic short
	*/
	public void addChord(int[] pitchArray, double rhythmValue /*duration*/, int dynamic){
		...
		for(;i<pitchArray.length;i++){
			Note newNote = new Note(pitchArray[i],rhythmValue/*duration*/,dynamic);
			((Phrase)this.phraseList.elementAt(i)).addNote(newNote);
		}
        // pad remaining phrases with rests
		for(;i<phraseList.size();i++){
			Note newNote = new Note(REST, rhythmValue/*duration*/);
			((Phrase)this.phraseList.elementAt(i)).addNote(newNote);
		}
	}
	...
	*/
	public void addChord(Note[] noteArray){
		this.currentTime = this.getEndTime();
		double rhythmValue/*duration*/ = noteArray[0].getRhythmValue(); // getDuration();
		//If we have more notes in the chord than we have available phrases
		//we will need to make more phrases
		...
		for(;i<noteArray.length;i++){
		    ...
		    noteArray[i].setRhythmValue(rhythmValue); /*setDuration(duration);*/
		...
		this.currentTime += rhythmValue; //duration;
	}
	...
	*/
    ...
    /**
	 * Return the value of the longest rhythm value (not duration) in the cphrase.
     */
    public double getLongestRhythmValue() {  // getLongestDuration() { JythonMusic
        double max = 0.0;
        Enumeration enum1 = getPhraseList().elements();
		while(enum1.hasMoreElements()){
			Phrase phrase = (Phrase) enum1.nextElement();
            //if(phrase.getLongestDuration() > max) max = phrase.getLongestDuration();
    ...
    /**
	 * Return the value of the shortest rhythm value (not duration) in the cphrase.
     */
    public double getShortestRhythmValue() { // getShortestDuration() { JythonMusic
        double min = 1000.0;
        Enumeration enum1 = getPhraseList().elements();
		while(enum1.hasMoreElements()){
			Phrase phrase = (Phrase) enum1.nextElement();
            //if(phrase.getShortestDuration() < min) min = phrase.getShortestDuration();
	...
	*/
	...
	public String toString(){
		String cphraseData = new String("<---- CPHRASE '" + title +
	...
	*/
	public void setRhythmValue(int val) { // setDuration(int val) { JythonMusic
		Enumeration enum1 = getPhraseList().elements();
		while(enum1.hasMoreElements()){
			Phrase phrase = (Phrase) enum1.nextElement();
		//phrase.setDuration(val); 					//JythonMusic extension
		phrase.setRhythmValue(val);
		}
	}

	/**
	* Change the Duration (not length) value of each note in the CPhrase.
	*/
	public void setDuration(double val) { // setLength(double val) { //JythonMusic extension
		Enumeration enum1 = getPhraseList().elements();
		while(enum1.hasMoreElements()){
			Phrase phrase = (Phrase) enum1.nextElement();
		//phrase.setLength(val);						//JythonMusic extension
